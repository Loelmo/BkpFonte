...
namespace Sistema_de_Gestao.User_Controls
{
    public partial class UCEstado : UCBase
    {
        ...
        public Boolean VerificaCamposObrigatoriosCadastro()
        {
            ...
            if (this.ObrigatorioTurma)
            {
                res &= Valida_DropDownList(DrpDwnLstTurma);
            }

            if (this.ObrigatorioEstado)
            {
                res &= Valida_DropDownList(DrpDwnLstEstado);
            }

            if (this.ObrigatorioEscritorioRegional)
            {
                res &= Valida_DropDownList(DrpDwnLstEscritorioRegional);
            }

            if (this.ObrigatorioRegiao)
            {
                res &= Valida_DropDownList(DrpDwnLstRegiao);
            }

            if (this.ObrigatorioCidade)
            {
                res &= Valida_DropDownList(DrpDwnLstCidade);
            }

            if (this.ObrigatorioBairro)
            {
                res &= Valida_DropDownList(DrpDwnLstBairro);
            }
