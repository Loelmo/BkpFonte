...
GO
ALTER PROCEDURE [dbo].[STP_SelecionaRankingFinalistaPorFiltro]  
(  
@CEA_TIPO_ETAPA INT,  
...
ISNULL((SELECT TOP 1 QFINAL.CDA_QUESTIONARIO_EMPRESA FROM TBL_QUESTIONARIO_EMPRESA QFINAL   
  ...
  AND QFINAL.TX_PROTOCOLO is Not Null), 0) [Vencedor],  
------------------------------------------------------------------------  
------------------------POS-VISITA--------------------------------------  
...
--------Relação com a Melhor da Categoria-------  
CASE WHEN @MAIORNOTAPOSVISITA > 0 THEN ((QE.NU_TOTAL_PONTUACAO_AVALIADOR * 100)/@MAIORNOTAPOSVISITA) ELSE 0 END [RelacaoComMelhorDaCategoriaPosVisita],  
------------------------------------------------------------------------  
------------------------PRE-VISITA--------------------------------------  
