USE [Sistema Gestao]
GO
/****** Object:  StoredProcedure [dbo].[STP_SelecionaRankingCandidataPorFiltro]    Script Date: 02/16/2011 11:07:50 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[STP_SelecionaRankingCandidataPorFiltro]
(
@CEA_TIPO_ETAPA INT,
...
ISNULL(QEA.CEA_USUARIO, 0) AS CEA_USUARIO_AVALIADOR_SENIOR,
ISNULL(QEA2.CEA_USUARIO, 0) AS CEA_USUARIO_AVALIADOR_ACOMPANHANTE,
ISNULL(U1.TX_USUARIO, '') AS NOME_USUARIO_AVALIADOR_SENIOR,
ISNULL(U2.TX_USUARIO, '') AS NOME_USUARIO_AVALIADOR_ACOMPANHANTE,
QE.FL_EXCLUIDO,
ET.CEA_ESTADO,
ET.FL_ATIVO AS FL_ETAPA_ATIVA,
ET.CDA_ETAPA,
--Possui Questionário de Responsabilidade Social----------       
ISNULL((SELECT TOP 1 1 FROM TBL_QUESTIONARIO_EMPRESA QRESP INNER JOIN TBL_ETAPA ET1 ON ET1.CDA_ETAPA = QRESP.CEA_ETAPA WHERE ET1.CEA_TIPO_ETAPA = @CEA_TIPO_ETAPA AND QRESP.CEA_EMP_CADASTRO = EC.CDA_EMP_CADASTRO AND QRESP.FL_ATIVO = 1 AND QRESP.CEA_QUESTIONARIO = @CEA_QUESTIONARIO_RESPONSABILIDADE_SOCIAL AND QE.TX_PROTOCOLO is Not Null), 0) [PossuiResponsabilidadeSocial],
----------------------------------------------------------
--Possui do Questionário de Inovação------------        
ISNULL((SELECT TOP 1 1 FROM TBL_QUESTIONARIO_EMPRESA QINOVA INNER JOIN TBL_ETAPA ET1 ON ET1.CDA_ETAPA = QINOVA.CEA_ETAPA WHERE ET1.CEA_TIPO_ETAPA = @CEA_TIPO_ETAPA AND QINOVA.CEA_EMP_CADASTRO = EC.CDA_EMP_CADASTRO AND QINOVA.FL_ATIVO = 1 AND QINOVA.CEA_QUESTIONARIO = @CEA_QUESTIONARIO_INOVACAO AND QE.TX_PROTOCOLO is Not Null), 0) [PossuiInovacao],
------------------------------------------------

-- Pontuação Responsabilidade Social
ISNULL((SELECT QEMP.NU_TOTAL_PONTUACAO FROM TBL_QUESTIONARIO_EMPRESA QEMP INNER JOIN TBL_ETAPA ET1 ON ET1.CDA_ETAPA = QEMP.CEA_ETAPA WHERE ET1.CEA_TIPO_ETAPA = @CEA_TIPO_ETAPA AND QEMP.CEA_EMP_CADASTRO = EC.CDA_EMP_CADASTRO AND QEMP.FL_ATIVO = 1 AND QEMP.CEA_QUESTIONARIO = @CEA_QUESTIONARIO_EMPREENDEDORISMO AND QE.TX_PROTOCOLO is Not Null), 0) [TotalResponsabilidadeSocial],
------------------------------------------------
-- Pontuação Inovacao
ISNULL((SELECT QEMPINOV.NU_TOTAL_PONTUACAO FROM TBL_QUESTIONARIO_EMPRESA QEMPINOV INNER JOIN TBL_ETAPA ET1 ON ET1.CDA_ETAPA = QEMPINOV.CEA_ETAPA WHERE ET1.CEA_TIPO_ETAPA = @CEA_TIPO_ETAPA AND QEMPINOV.CEA_EMP_CADASTRO = EC.CDA_EMP_CADASTRO AND QEMPINOV.FL_ATIVO = 1 AND QEMPINOV.CEA_QUESTIONARIO = @CEA_QUESTIONARIO_INOVACAO AND QE.TX_PROTOCOLO is Not Null), 0) [PontuacaoInovacao],
----------------------------------------------------------------------------------------
--Verifica se a Empresa é Finalista do Questionário de Responsabilidade Social----------       
ISNULL((SELECT TOP 1 QFINAL.CDA_QUESTIONARIO_EMPRESA FROM TBL_QUESTIONARIO_EMPRESA QFINAL 
...
ISNULL((SELECT TOP 1 QEMP.NU_TOTAL_PONTUACAO_AVALIADOR FROM TBL_QUESTIONARIO_EMPRESA QEMP WHERE QEMP.CEA_EMP_CADASTRO = EC.CDA_EMP_CADASTRO AND QEMP.FL_ATIVO = 1 AND QEMP.CEA_QUESTIONARIO = @CEA_QUESTIONARIO_EMPREENDEDORISMO AND QE.TX_PROTOCOLO is Not Null), 0) [TotalEmpreendedorismoPosVisita],
---------------------------------------------------
...
ISNULL((SELECT TOP 1 QEMPRESP.NU_TOTAL_PONTUACAO_AVALIADOR FROM TBL_QUESTIONARIO_EMPRESA QEMPRESP WHERE QEMPRESP.CEA_EMP_CADASTRO = EC.CDA_EMP_CADASTRO AND QEMPRESP.FL_ATIVO = 1 AND QEMPRESP.CEA_QUESTIONARIO = @CEA_QUESTIONARIO_RESPONSABILIDADE_SOCIAL AND QE.TX_PROTOCOLO is Not Null), 0) [PontuacaoTotalResponsabilidadeSocial],
---------------------------------------------------
--Pontuação do Questionário de Inovacao----        
ISNULL((SELECT TOP 1 QEMPINOV.NU_TOTAL_PONTUACAO_AVALIADOR FROM TBL_QUESTIONARIO_EMPRESA QEMPINOV WHERE QEMPINOV.CEA_EMP_CADASTRO = EC.CDA_EMP_CADASTRO AND QEMPINOV.FL_ATIVO = 1 AND QEMPINOV.CEA_QUESTIONARIO = @CEA_QUESTIONARIO_INOVACAO AND QE.TX_PROTOCOLO is Not Null), 0) [PontuacaoTotalInovacao],
