...
namespace Sistema_de_Gestao.MPE.Paginas
{
    public partial class InscricaoAdminCE : PaginaBase
    {
        ...
        {
            ListaGrid = new BllTurmaEmpresa().ObterTodasInscritasPorFiltros(objTurmaEmpresa, objGrupo, sProtocolo, dDataInicio, dDataFim, EntTipoEtapa.TIPO_ETAPA_FGA_INSCRICAO_AUTODIAGNOSTICO_ADMINISTRATIVO);
        ...
        private void PopulaStatus()
        {
            ...
            item.Value = EntTipoEtapa.TIPO_ETAPA_FGA_VALIDACAO_RESPOSTAS.ToString();
            ddlStatus.Items.Add(item);

            //Insere Item Classificada
            item = new ListItem();
            item.Text = "Pré-Classificada";
            item.Value = EntTipoEtapa.TIPO_ETAPA_FGA_PRE_CLASSIFICADAS.ToString();
            ddlStatus.Items.Add(item);

            //Insere Item Classificada
            item = new ListItem();
            item.Text = "Classificada";
            item.Value = EntTipoEtapa.TIPO_ETAPA_FGA_FASE_4.ToString();
            ddlStatus.Items.Add(item);
        ...
        private void PopulaTipoEmpresa()
        {
            ...
            this.ddlTipoEmpresa.DataSource = new BllTipoEmpresa().ObterTodosFga();
