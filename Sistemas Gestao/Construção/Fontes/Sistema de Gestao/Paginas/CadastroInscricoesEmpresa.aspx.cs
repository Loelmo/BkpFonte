    ...
    public partial class CadastroInscricoesEmpresa : PaginaBase
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                RdBttnLstPergunta1.SelectedIndex = -1;
                RdBttnLstPergunta2.SelectedIndex = -1;
                RdBttnLstPergunta3.SelectedIndex = -1;
                RdBttnLstPergunta4.SelectedIndex = -1;
                tabelaResposta.Style.Add("display", "none");
                ...
                if (this.EmpresaLogada != null && this.EmpresaLogada.IdEmpresaCadastro > 0)
                {
                    ...
                    if (objTurmaEmpresa != null)
                    {
                        ...
                        ChkBxAceitoParticipar.Checked = true;
            ...
            if (this.objPrograma.IdPrograma == EntPrograma.PROGRAMA_MPE)
            {
                celulaDestaques1.Style.Add("display", "block");
                celulaDestaques2.Style.Add("display", "block");
                celulaDestaques3.Style.Add("display", "block");
                celulaAceito.Style.Add("display", "block");
                Label49.Style.Add("display", "block");
                ChkBxAceitoParticipar.Style.Add("display", "block");

                RdBttnLstDestaque2.Style.Add("display", "block");
                Label19.Style.Add("display", "block");
                RdBttnLstDestaque1.Style.Add("display", "block");
                Label44.Style.Add("display", "block");
                RdBttnLstDestaque3.Style.Add("display", "block");
                HprLnk1.Style.Add("display", "block");

                tblQuestoesDestaque.Visible = true;
            }
            else
            {
                Label49.Style.Add("display", "none");
                ChkBxAceitoParticipar.Style.Add("display", "none");

                RdBttnLstDestaque2.Style.Add("display", "none");
                Label19.Style.Add("display", "none");
                RdBttnLstDestaque1.Style.Add("display", "none");
                Label44.Style.Add("display", "none");
                RdBttnLstDestaque3.Style.Add("display", "none");
                HprLnk1.Style.Add("display", "none");

                tblQuestoesDestaque.Visible = false;
            }
            TxtBxNumeroEmpregados.Focus();
        }
        ...
        protected void ImgBttnProcessar2_Click(object sender, ImageClickEventArgs e)
        {
            ...
            this.Etapa3.Visible = true;
            this.RdBttnLstPergunta1.Focus();
        }
...
//            res &= Valida_TextBoxSenha(TxtBxSenha, TxtBxConfSenha);
        ...
        private void Gravar()
        {
            ...
            try
            {
                ...
                {
                    if (this.RdBttnLstDestaque1.SelectedValue == "1")
                    {
                        AlteraParticipacao(EntQuestionario.QUESTIONARIO_RESPONSABILIDADE_2011, true);
                    }
                    else
                    {
                        AlteraParticipacao(EntQuestionario.QUESTIONARIO_RESPONSABILIDADE_2011, false);
                    }
                    if (this.RdBttnLstDestaque2.SelectedValue == "1")
                    {
                        AlteraParticipacao(EntQuestionario.QUESTIONARIO_INOVACAO_2011, true);
                    }
                    else
                    {
                        AlteraParticipacao(EntQuestionario.QUESTIONARIO_INOVACAO_2011, false);
                    }
                    if (this.RdBttnLstDestaque3.SelectedValue == "1")
                    {
                        AlteraParticipacao(EntQuestionario.QUESTIONARIO_EMPREENDEDORISMO_2011, true);
                    }
                    else
                    {
                        AlteraParticipacao(EntQuestionario.QUESTIONARIO_EMPREENDEDORISMO_2011, false);
                    }
                }
        ...
        private void ObjectToPage(EntInscricoesEmpresa objInscricoesEmpresa)
        {
            ...
            if (objPrograma.IdPrograma == EntPrograma.PROGRAMA_MPE)
            {
                ...
                foreach (EntQuestionario eq in listaQuestionarios)
                {
                    if (eq.IdQuestionario == EntQuestionario.QUESTIONARIO_INOVACAO_2011)
                    {
                        ...
                        {
                            RdBttnLstDestaque2.SelectedValue = "1";
                        }
                        else
                        {
                            RdBttnLstDestaque2.SelectedValue = "0";
                        }
                    }
                    else
                    {
                        RdBttnLstDestaque2.SelectedValue = "";
                    }

                    if (eq.IdQuestionario == EntQuestionario.QUESTIONARIO_RESPONSABILIDADE_2011)
                    {
                        if (eq.EmpresaParticipa)
                        {
                            RdBttnLstDestaque1.SelectedValue = "1";
                        }
                        else
                        {
                            RdBttnLstDestaque1.SelectedValue = "0";
                        }
                    }
                    else
                    {
                        RdBttnLstDestaque1.SelectedValue = "";
                    }

                    if (eq.IdQuestionario == EntQuestionario.QUESTIONARIO_EMPREENDEDORISMO_2011)
                    {
                        if (eq.EmpresaParticipa)
                        {
                            RdBttnLstDestaque3.SelectedValue = "1";
                        }
                        else
                        {
                            RdBttnLstDestaque3.SelectedValue = "0";
                        }
                    }
                    else
                    {
                        RdBttnLstDestaque3.SelectedValue = "";
                    }
                }
            }*/
            ...
            this.PopulaParticipaPremiosEspeciais();
        ...
        private void PopulaEstado()
        {
            ...
            foreach (EntEstado objEstado in new BllEstado().ObterEstadosPorTurma(objTurma.IdTurma, EntTipoEtapa.TIPO_ETAPA_MPE_INSCRICAO_CANDIDATURA_EMPRESA))
            {
                item = new ListItem();
        ...
        private void PopulaParticipaPremiosEspeciais()
        {
            EntQuestionarioEmpresa qe = new BllQuestionarioEmpresa().ObterQuestionarioAberto(EntQuestionario.QUESTIONARIO_EMPREENDEDORISMO_2011, EmpresaLogada.IdEmpresaCadastro, objTurma.IdTurma);
            
            if (qe != null)
            {
                this.RdBttnLstDestaque1.SelectedValue = "1";
            }
            else
            {
                this.RdBttnLstDestaque1.SelectedValue = "0";
            }

            qe = new BllQuestionarioEmpresa().ObterQuestionarioAberto(EntQuestionario.QUESTIONARIO_INOVACAO_2011, EmpresaLogada.IdEmpresaCadastro, objTurma.IdTurma);
            if (qe != null)
            {
                this.RdBttnLstDestaque2.SelectedValue = "1";
            }
            else
            {
                this.RdBttnLstDestaque2.SelectedValue = "0";
            }

            qe = new BllQuestionarioEmpresa().ObterQuestionarioAberto(EntQuestionario.QUESTIONARIO_RESPONSABILIDADE_2011, EmpresaLogada.IdEmpresaCadastro, objTurma.IdTurma);
            if (qe != null)
            {
                this.RdBttnLstDestaque3.SelectedValue = "1";
            }
            else
            {
                this.RdBttnLstDestaque3.SelectedValue = "0";
            }
        }
        ...
        private void PassoAPasso(Int32 Etapa)
        {
            ...
            else if (Etapa == 3)
            {
                if (TxtBxSenha.Text != "")
                {
                    this.HddnFldSenha.Value = StringUtils.EncryptPassword(this.TxtBxSenha.Text);
                }
                this.Etapa1.Visible = false;
                this.Etapa2.Visible = false;
                this.Etapa3.Visible = true;
        ...
        protected void BtnProximo_Click(object sender, EventArgs e)
        {
            if (VerificaCamposObrigatoriosEmpresa())
            {
                ...
                Boolean FaturamentoAcima = false;
                if (StringUtils.ToDate(this.TxtBxDataAbertura.Text) > StringUtils.ToDate("31/12/2009"))
                {
                    DataPosterior = true;
                }
                if (this.CmbBxFaturamento.SelectedValue == "52" || this.CmbBxFaturamento.SelectedValue == "53")
                {
                    FaturamentoAcima = true;
                }
                if (DataPosterior && FaturamentoAcima)
                {
                    MessageBox(this, "Somente empresas com o Faturamento abaixo de R$ 2,4 milhões e Data de Abertura anterior a 31/12/2009 podem participar do Prêmio MPE Brasil 2011!");
                }
                else if (DataPosterior)
                {
                    MessageBox(this, "Somente empresas com Data de Abertura anterior a 31/12/2009 podem participar do Prêmio MPE Brasil 2011!");
                }
                else if (FaturamentoAcima)
                {
                    MessageBox(this, "Somente empresas com o Faturamento abaixo de R$ 2,4 milhões podem participar do Prêmio MPE Brasil 2011!");
                }
            	this.PassoAPasso(2);
            }
            else
        ...
        {
            if (this.objPrograma.IdPrograma == EntPrograma.PROGRAMA_MPE && !ChkBxAceitoParticipar.Checked)
            {
                MessageBox(this, "Você não selecionou a opção: Aceito participar do MPE Brasil, conforme regulamento do Prêmio, Ciclo 2010, disponível no Portal www.premiompe.sebrae.com.br.");
            }
            else
            {
            ...
            if (cepTemp.Length == 8)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        MessageBox(this.Page, "Este CEP é de um Estado que não está participando do Prêmio MPE 2011.");
